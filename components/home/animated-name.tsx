...
// Font definitions - FIXED: Each font must be declared separately at module level
...
// Then reference them in an object if needed
const fonts = {
...
// Identity states with matching colors to the hero section gradient
const nameStates = [
  {
    font: fonts.orbitron,
...
export default function AnimatedName() {
  ...
  const particleOptions = useMemo(
    () => ({
      ...
      particles: {
        ...
        color: {
          // Match particle colors to theme of current state and hero gradient
        ...
        move: {
          ...
          speed: 15, // Slightly slower particles
          direction: "outside",
          random: false,
          straight: false,
          outModes: {
  ...
  return (
    <div className="relative w-full">
      {/* Container for particles with reduced height to align better with hero layout */}
      ...
      {/* Content container - improved alignment with hero layout */}
      <div className="flex flex-col items-center lg:items-start">
        <motion.h1
          ...
          aria-label={`Grant McNatt - ${nameStates[currentState].description}`}
        >
          Grant McNatt
        </motion.h1>

        {/* Subtitle with improved spacing for hero layout */}
        <motion.div
          initial={{ opacity: 0, scale: 0.8 }}
          animate={{ opacity: 1, scale: 1 }}
          key={currentState} // Force re-render on state change
          transition={{
            duration: 0.3,
            staggerChildren: 0.1,
          }}
          className="flex items-center gap-2 mt-1 sm:mt-2" // Reduced margin to work better with hero text
        >
          <FontAwesomeIcon
